...
const MovieDetails = props => {
    ...
    return (
        <Container bgColor={materialColors.blackPrimary}>
            <ScrollView>
                <Slider
                    images={[`${MOVIE_POSTER_HOST}${data.poster_path}`]}
                    onCurrentImagePressed={onPlay}>
                    {slider => (
                        <Button
                            name="play-circle"
...
function VideoModal({ close, ...props }) {
    return (
        <Modal transparent animationType="fade" {...props}>
            <Video
                source={{
                    uri: 'https://player.vimeo.com/external/434945712.sd.mp4?s=433fbe2cba95823e978d76d17ad03ec4f001bd55&profile_id=165&oauth2_token_id=57447761',
                }}
            />
        </Modal>
    )
